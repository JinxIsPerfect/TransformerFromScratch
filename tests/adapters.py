...
def run_swiglu(
    ...
    from transformer.model import SwiGLU
...
def run_scaled_dot_product_attention(
    ...
    """
    from transformer.model import DotProductAttention
...
def run_multihead_self_attention(
    ...
    """
    from transformer.model import CausalMultiHeadSelfAttention
    mha = CausalMultiHeadSelfAttention(d_model, num_heads, device=in_features.device)
...
def run_multihead_self_attention_with_rope(
    ...
    """
    from transformer.model import CausalMultiHeadSelfAttention
    attn = CausalMultiHeadSelfAttention(d_model, num_heads, use_rope=True, max_seq_len=max_seq_len, theta=theta,device=in_features.device)
    attn.load_state_dict({"q_proj.weight":q_proj_weight,
                          "k_proj.weight":k_proj_weight,
                          "v_proj.weight":v_proj_weight,
                          "output_proj.weight":o_proj_weight})
...
def run_transformer_block(
    ...
    """
    from transformer.model import TransformerBlock
    block = TransformerBlock(d_model, num_heads, d_ff, use_rope = True, max_seq_len=max_seq_len, theta=theta, device=in_features.device,dtype=in_features.dtype)
    block.load_state_dict(weights)
    B, S, _ = in_features.shape
    token_positions = torch.arange(S, device=in_features.device).expand(B,S)
...
def run_cross_entropy(
    ...
    """
    from transformer.nn_utils import CrossEntropy
...
def run_gradient_clipping(parameters: Iterable[torch.nn.Parameter], max_l2_norm: float) -> None:
    ...
    """
    from transformer.nn_utils import gradient_clipping
